...
--------------------------------------------------------
  ...
  CREATE OR REPLACE PACKAGE BODY "PRJ"."CSS_PCK" as
...
PERSONNEL_NAME, PERSONNEL_ACTIVE_FLG, CUSTOMER_CODE, CUSTOMER_NAME,
LOCATION, decode(task_code,'13','PM', decode(task_code,'04','DEV', task_code)), TASK_DESC, PROJECT_CODE, MODULE_CODE, MODULE_NAME,
LABOUR_HOURS, description
from CSS_PROJECT_TASKS_V@CSS254 where task_code is not null
and schedule_date > '31-DEC-2018'
and customer_code = nvl(Cust_Code_P, customer_code);
--
...
begin
  ...
  insert into tareif(c1) values(c1r.task_desc||'-'||c1r.task_code);commit;
