...
set define off;
  ...
  CREATE OR REPLACE PROCEDURE "PRJ"."GENERATE_OFFER_FROM_OPP" (opp_id_p  NUMBER)
is   
...
select PRJ_PPM_SEQ.NEXTVAL into pipe_id from dual;
   insert into lgs_contracts(ID,DSP_CODE ,COMPANY_CODE, CONTRACT_TYPE_ID,PRICING_DATE, WHO_CREATED,  ACCOUNT_MANAGER, USAGE_TYPE ,PROJECT_NAME, OFFER_STATUS ,REMARKS,DESCRIPTION)
   values ( cntrct_id,c1r.OPP_CODE ,c1r.OPP_CUSTOMER, c1r.OPP_CONTRACT_TYPE,SYSDATE, c1r.orl_employee,  c1r.orl_employee, 1 ,c1r.OPP_PROJECT_NAME , 1,c1r.OPP_REMARKS,c1r.opp_desc);
 open c2;  
    LOOP
        fetch c2 into c2r;
